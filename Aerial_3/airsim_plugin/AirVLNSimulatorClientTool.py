...
class AirVLNSimulatorClientTool:
    ...
    def getImageResponses(
        self, cameras=['0', '1', '2', '3'], poses=None, warmup_renderer=False
    ):
        def _getImages(airsim_client: airsim.VehicleClient):
            ...
            if warmup_renderer:
                # A newly opened off-screen Unreal scene can expose its AirSim
                # RPC port before it has produced an RGB frame. Advance it in
                # real time and keep it running through the first image RPC;
                # requesting that first frame while paused can block until the
                # RPC timeout on some off-screen Unreal builds.
                airsim_client.simPause(False)
                time.sleep(1.0)
            ...
            while True:
                try:
                    ...
                    for idx, camera_name in enumerate(cameras):
                        ...
                        decoded_rgb = cv2.imdecode(
                            np.frombuffer(image, dtype=np.uint8),
                            cv2.IMREAD_COLOR,
                        )
                        if decoded_rgb is None:
                            raise RuntimeError(
                                f'camera {camera_name} returned an invalid RGB image'
                            )
                        if not np.any(decoded_rgb):
                            raise RuntimeError(
                                f'camera {camera_name} returned an all-black RGB frame; '
                                'waiting for the Unreal renderer'
                            )
                    ...
                    # Waiting while paused cannot repair an all-black startup
                    # frame. Keep rendering active through the next image RPC.
                    try:
                        ...
                        logger.error("渲染恢复错误: " + str(resume_error))
