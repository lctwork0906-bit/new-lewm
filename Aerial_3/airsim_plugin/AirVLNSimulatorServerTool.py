...
def create_drones(drone_num_per_env=1, show_scene=False, uav_mode=True) -> dict:
    ...
    # Optional evaluation-only capture sizes.  Defaults preserve the original
    # 512 px RGB / 256 px depth behavior for every existing launcher.
    rgb_size = int(os.environ.get("AIRSIM_RGB_SIZE", "512"))
    depth_size = int(os.environ.get("AIRSIM_DEPTH_SIZE", "256"))
    for vehicle in airsim_settings["Vehicles"].values():
        for camera in vehicle.get("Cameras", {}).values():
            for capture in camera.get("CaptureSettings", []):
                size = rgb_size if capture.get("ImageType") == 0 else depth_size
                capture["Width"] = size
                capture["Height"] = size
    ...
    def _open_scenes(self, ip: str , scen_id_gpu_list: list):
        ...
        for index, (scen_id, gpu_id) in enumerate(scen_id_gpu_list):
            ...
            if choose_env_exe_paths[index] is None:
                ...
                subprocess_execute = [
                    "taskset", "-c", args.cpu_affinity,
                    "bash", choose_env_exe_paths[index],
                    "-RenderOffscreen", "-NoSound", "-NoVSync",
                    "-GraphicsAdapter={}".format(gpu_id),
                    "--settings={}".format(
                        str(CWD_DIR / 'settings' / str(ports[index]) / 'settings.json')
                    ),
                ]
                time.sleep(3)
                print(shlex.join(subprocess_execute))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--root_path",
        type=str,
        default=str(Path(__file__).resolve().parents[2] / "TEST_ENVS"),
    ...
    parser.add_argument(
        ...
        type=str,
        default="0-7",
        help='CPU cores assigned to each Unreal scene process (taskset syntax)'
    )
